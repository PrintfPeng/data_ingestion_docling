...
- [NEW] ใช้ Gemini ช่วยสรุปใจความสำคัญของตาราง (Summarization)
...
# -------------------------------
# Helper: Gemini Table Summarizer
# -------------------------------

def _get_gemini_model():
    """เตรียม Gemini สำหรับสรุปตาราง"""
...
def _summarize_table(model, columns: list, rows: list) -> str:
    """ส่งข้อมูลตารางให้ AI สรุป"""
    if not model:
        return ""
    
    # แปลงตารางเป็น Text format ง่ายๆ
    table_str = f"Columns: {', '.join(columns)}\nRows:\n"
    # ตัดแค่ 10 แถวแรกพอประหยัด token (ให้ AI เห็นภาพรวม)
    for r in rows[:10]: 
        table_str += f"- {', '.join(map(str, r))}\n"
...
def extract_tables(
    ...
    """
    ดึงตารางจาก PDF 1 ไฟล์ทั้งหมด และทำสรุปด้วย AI
    ...
    for flavor in flavor_priority:
        ...
        for t in tables:
            ...
            summary_text = ""
            if gemini_model:
                print(f"[table_extractor] Summarizing table {table_id}...")
                summary_text = _summarize_table(gemini_model, columns, rows)
            ...
            table_block = TableBlock(
                ...
                extra={
                    ...
                    "summary": summary_text, # เก็บผลสรุปลง Metadata
                },
            )
